...
class DatasetWrapper(object):
	...
	class __DatasetWrapper(object):
		...
		def __init__(self, params):	
			...
			datapath = '../../data/kvasir-seg'
			folder = os.path.join(datapath, 'images')
			if os.path.isdir(folder):
				self.dataset = [[datapath, image] for image in os.listdir(folder) if '.jpg' in image]
	...
	def features(self, key):
		...
		return os.path.join(path, 'images', image)

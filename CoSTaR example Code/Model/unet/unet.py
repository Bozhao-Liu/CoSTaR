...
'''
def Unet(num_classes, pretrained=True, scale=1):
	"""
	...
	net.inc = (DoubleConv(1, 64))
	if num_classes != 2:
		net.outc = (OutConv(64, num_classes))
	return net
'''
def Unet(in_channels=3, num_classes = 1, pretrained=True):
	#https://www.sciencedirect.com/science/article/pii/S0010482519301520
	if in_channels == 1: 
		model = torch.hub.load('mateuszbuda/brain-segmentation-pytorch', 'unet', in_channels=3, out_channels=num_classes, init_features=32, pretrained=pretrained)
		conv = model.encoder1.enc1conv1
		w = conv.weight.data			# [32, 3, 3, 3]

		# luminance-weighted conversion (better than mean)
		w_gray = (
			0.2989 * w[:, 0:1] +
			0.5870 * w[:, 1:2] +
			0.1140 * w[:, 2:3]
		)
		new_conv = torch.nn.Conv2d(1, conv.out_channels,
		...
		model.encoder1.enc1conv1 = new_conv
		return model
	else:
		return torch.hub.load('mateuszbuda/brain-segmentation-pytorch', 'unet',
					in_channels=in_channels, out_channels=num_classes, init_features=32, pretrained=pretrained)
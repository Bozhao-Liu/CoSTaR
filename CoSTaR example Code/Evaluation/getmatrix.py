...
# S-MEASURE
# Based on:
# Fan et al., "Structure-measure: A New Way to Evaluate
...
def _object_score(pred, gt):
    """
    Object-aware similarity component.
    """
    x = pred[gt == 1]
    ...
    return (2.0 * mean_x) / (mean_x**2 + 1.0 + std_x + eps)


def _ssim_simple(pred, gt):
    """
    Simplified SSIM used in original S-measure paper.
    """
    ...
    var_pred = ((pred - mean_pred) ** 2).sum() / (N - 1 + eps)
    var_gt = ((gt - mean_gt) ** 2).sum() / (N - 1 + eps)

    cov = (
        ((pred - mean_pred) * (gt - mean_gt)).sum()
        / (N - 1 + eps)
    )

    alpha = 4 * mean_pred * mean_gt * cov
    beta = (
        (mean_pred**2 + mean_gt**2)
        * (var_pred + var_gt)
    )
...
def _region_score(pred, gt):
    ...
    q1 = _ssim_simple(pr1, gt1)
    q2 = _ssim_simple(pr2, gt2)
    q3 = _ssim_simple(pr3, gt3)
    q4 = _ssim_simple(pr4, gt4)
...
def compute_s_measure_single(gt_mask, pred_mask, alpha=0.5):
    """
    Compute S-measure for a single mask pair.

    Inputs:
        gt_mask   : binary GT mask
        pred_mask : binary OR probability prediction

    Returns:
        float
    """

    gt = _ensure_single_mask(gt_mask).astype(np.float64)
    ...
    # Completely foreground GT
    if y == 1:
        return pred.mean()

    # Object-aware score
    fg_score = _object_score(pred, gt)
    bg_score = _object_score(1 - pred, 1 - gt)

    object_score = y * fg_score + (1 - y) * bg_score

    # Region-aware score
    region_score = _region_score(pred, gt)

    s_measure = alpha * object_score + (1 - alpha) * region_score

    return max(0.0, s_measure)
...
def compute_segmentation_metrics(gt_masks, pred_masks, thresholds=None):
    ...
    # --------------------------------------------------
    # Keep 2D versions for boundary + S-measure
    # --------------------------------------------------
    gt_masks_2d = gt_masks.clone()
    pred_masks_prob_2d = pred_masks.clone()

    # --------------------------------------------------
    # Best threshold by IoU
    # --------------------------------------------------
    if thresholds is None:
        thresholds = torch.round(
            torch.linspace(0.1, 0.9, 51, device=device),
            decimals=2
        )

    best_iou, best_threshold = get_best_threshold(
        gt_masks,
        pred_masks,
        thresholds
    )
    ...
    # Binary version for S-measure
    pred_masks_2d = pred_masks.clone()
    ...
    # --------------------------------------------------
    AP50, AR50 = precision_recall_at_threshold(
        gt_masks,
        pred_masks,
        IOU_threshold=0.5
    )

    AP30, AR30 = precision_recall_at_threshold(
        gt_masks,
        pred_masks,
        IOU_threshold=0.3
    )

    AP, AR = precision_recall_at_threshold(
        gt_masks,
        pred_masks,
        IOU_threshold=0
    )
    ...
    # Coverage metrics
    # --------------------------------------------------
    precision, recall, miss, overflow = compute_coverage(
        gt_masks,
        pred_masks
    )
    ...
    for i in range(N):
        ...
        s = compute_s_measure_single(
            gt_masks_2d[i],
            pred_masks_prob_2d[i]
        )

        S_measure.append(s)

    result['S-measure'] = float(np.mean(S_measure))

...
class DatasetWrapper(object):
	...
	class __DatasetWrapper(object):
		...
		def __init__(self, params):	
			...
			datapath = '../../data/HAM1000'
			folder = os.path.join(datapath, 'masks')
			if os.path.isdir(folder):
				self.dataset = [[datapath, image] for image in os.listdir(folder) if '.png' in image and os.path.isfile(os.path.join(datapath, 'images', image.replace(".png", ".jpg")))]
			...
			usable_len = (len(self.ind) // self.cv_iters) * self.cv_iters

			# Portion to shuffle (first cv_iters - 1 folds)
			shuffle_len = usable_len * (self.cv_iters - 1) // self.cv_iters

			# Slice
			first_part = self.ind[:shuffle_len].copy()   # copy needed for safe shuffling
			last_part  = self.ind[shuffle_len:usable_len]

			# Shuffle first part in-place
			np.random.shuffle(first_part)

			# Reassemble
			self.ind = np.concatenate([first_part, last_part], axis=0)
	...
	def features(self, key):
		...
		Returns:
			features in list	
		"""
		path, image = DatasetWrapper.instance.dataset[key]
		return os.path.join(path, 'images', image.replace(".png", ".jpg"))
	...
	def label(self, key):
		...
		"""
		path, image = DatasetWrapper.instance.dataset[key]
		return os.path.join(path, 'masks', image )
	...
	def box(self, key):
		...
		"""
		path, image = DatasetWrapper.instance.dataset[key]
		return os.path.join(path, 'boxes', image)
...
class Normalize(torch.nn.Module):
    ...
    def forward(self, tensor: Tensor) -> Tensor:
        return (tensor - torch.min(tensor))/(torch.max(tensor)-torch.min(tensor)) if (torch.max(tensor)-torch.min(tensor)) > 0 else torch.zeros(tensor.shape)
...
class imageDataset(Dataset):
	...
	def __init__(self, dataSetType, params, CViters):
		...
		self.mask_transformer = transforms.Compose([
			...
			transforms.Lambda(lambda t: t.round().to(torch.uint8))
		])
	...
	def __getitem__(self, idx):
		...
		label = self.mask_transformer(label).to(torch.uint8)
